...
export default {
    ...
    methods: {
        ...
        yhlbmkGetList(currentPage, isSearch) {
            ...
            axios.get('/api/auction/task/sys/findList')
            	.then( (response)=> {
		...
		yhlbmkOk(){
			...
			if(this.yhlbmkAddObj.pic.length==0 || this.yhlbmkAddObj.reward=='' || this.yhlbmkAddObj.description=='' || this.yhlbmkAddObj.taskNum=='' || this.model1===''){
				debugger
				Util.error('正确填写表单');
...
// 			formData.append('scheduleOk', this.yhlbmkAddObj.taskNum);
			...
			if(this.yhlbmkAddObj.reward=='' || this.yhlbmkAddObj.taskNum=='' || this.yhlbmkAddObj.description==''){
				Util.error('正确填写表单');
				this.yhlbmkLoading = false; // 关闭加载状态
			}else{
			axios.post('/api/auction/task/sys/addOrUpdate',formData, config)
				.then( (response)=> {
					console.log(response)
					if(response.data.code==200){
						Util.success("修改成功");
